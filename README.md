...
쏘카의 유즈케이스에 맞는 Airflow를 위해 Fork해서 사용합니다.

[이전 README](./README_socar_legacy.md)


## Development

1. `socar` 브랜치를 default 브랜치로 사용합니다.
   1. feature branch를 만들어 socar 브랜치에 push 합니다.
   2. 별도의 release가 필요한 경우, tag를 만들어 사용합니다.
2. upstream (apache/airflow)와 싱크가 필요할 때는 데이터 엔지니어링팀 혹은 @socar-humphrey 에게 요청합니다.

## Docker Image Build

Github Action을 이용해 Airflow docker image를 빌드합니다.
Workflow dispatch 기능을 통해 원하는 Airflow, python 버전 그리고 추가적으로 설치할 pip package 등을 입력해서 GCR에 push 해주세요.
